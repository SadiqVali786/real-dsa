...
class BaseGraph:
    ...
    def _reconstruct_path(self, source_node: int, destination_node: int, visited: list[bool], parent: list[int]):
        """
            Reconstructs path from source to destination by following parent pointers backwards.

            Args:
                source_node: Starting node
                destination_node: Target node
                visited: Boolean array indicating visited nodes
                parent: Parent array where parent[i] = j means j is parent of i
            ...
            Edge Cases:
                - Destination not reachable from source (returns empty list)
                - Source equals destination (returns [source])
        """
        ...
        # Safety check: prevent infinite loop if parent chain is broken
        max_path_length = self.V
        steps = 0
        while current_node != source_node:
            if steps >= max_path_length:
                # Parent chain is broken, return empty path
                return []
...
class DirectedGraph(BaseGraph):
    ...
    def bellman_ford(self, source_node: int, destination_node: int):
        ...
        for iteration in range(self.V - 1):  # O(V-1) iterations for relaxation
            relaxed: bool = False  
                ...
                if distance[node] != math.inf:
                    for weight, neighbor in self._adjacency_list[node]:  # O(degree(node)) = O(E) total
                        ...
                        if new_distance < distance[neighbor]:  
                            ...
                            relaxed = True 
            # Early termination optimization: if no relaxation, we're done
            if not relaxed:
                break
