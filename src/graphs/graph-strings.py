...
class BaseGraph:
    ...
    def _reconstruct_path(self, source_node: str, destination_node: str, visited: set[str], parent: dict[str, str | None]):
        ...
        while current_node != source_node:
            ...
            # Safety check: if parent is None and we haven't reached source, path is invalid
    ...
    def floyd_warshall(self):
        """
            ...
            Returns:
                tuple: (distance_matrix, parent_matrix, has_negative_cycle, negative_cycle_nodes) -
                distance_matrix[i][j] is shortest distance from node i to j, parent_matrix for path reconstruction,
                boolean flag for negative cycles, list of affected nodes. Returns ([], [], False, []) if graph is empty
        ...
        # Cycle detection: Check for negative cycles using Floyd-Warshall
        # A negative cycle exists if distance_matrix[i][i] < 0 for any node i
        # This means we can reach node i from itself with a negative total weight
        has_negative_cycle = False
        negative_cycle_nodes: list[str] = []
        
        # Check diagonal elements for negative values => O(V) TC
        for i in range(num_nodes):
            if distance_matrix[i][i] < 0:
                has_negative_cycle = True
                node = self._index_to_node[i]
                negative_cycle_nodes.append(node)
        
        # If negative cycles exist, mark all nodes reachable from negative cycle nodes
        # A node j is affected by a negative cycle if distance_matrix[i][j] != inf for some i in negative_cycle_nodes
        if has_negative_cycle:
            affected_nodes: set[str] = set(negative_cycle_nodes)
            for cycle_node_idx in [self._node_to_index[node] for node in negative_cycle_nodes]:
                for j in range(num_nodes):
                    if distance_matrix[cycle_node_idx][j] != math.inf:
                        affected_nodes.add(self._index_to_node[j])
            negative_cycle_nodes = sorted(affected_nodes)
        
        return distance_matrix, parent_matrix, has_negative_cycle, negative_cycle_nodes
